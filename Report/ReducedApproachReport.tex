...
\begin{align}
\min\limits_{\vec{y},\vec{u}\in\Omega} J(\vec{y},\vec{u}) &:= \frac{1}{2}\ \norm{\vec{y}-\vec{y}_d}
_{L^2(\Omega)}^2 + \frac{\nu}{2}\ \norm{\vec{u}}_{L^2(\Omega)}^2\\
...
\begin{equation*}
||\vec{x}||_{L^2(\Omega)}^2 := h^2 \sum\limits_{j=1}^n \vec{x}_j^2
\end{equation*}
such that
\begin{equation*}
A\vec{y}=\vec{f}+\vec{u}
...
We see that
\begin{equation*}
||\vec{x}||_{L^2(\Omega)}^2 = h^2\ ||\vec{x}||_2^2
\end{equation*}
...
&=\ \frac{1}{2}\ h^2 ||A^{-1}\left(\vec{f}+\vec{u}\right)-\vec{y}_d||_2^2\ + \frac{\nu}{2}\ h^2 ||\vec{u}||_2^2\\
